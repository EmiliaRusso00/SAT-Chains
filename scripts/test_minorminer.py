...
# ---------------------------------------------------------
# RUN MINORMINER PER VERSIONE FULL
# ---------------------------------------------------------
def run_minorminer(G_logical, G_physical, exp, mode, out_dir, attempt, max_attempts):
    ...
    if success:
        ...
        print(f"[MM Success {attempt_str} | {mode.upper()}] "
              f"Max chain: {max_chain} | Num fisici usati: {num_phys_used} | Tempo: {elapsed:.4f}s")
    else:
    ...
    if embedding:
        ...
        logical_edges, chain_edges = compute_used_physical_edges(G_logical, G_physical, embedding)
        result["physical_edges_logical"] = list(logical_edges)
        result["physical_edges_chain"] = list(chain_edges)

    os.makedirs(out_dir, exist_ok=True)
    with open(os.path.join(out_dir, f"minorminer_result.json"), "w") as f:
...
def main():
    ...
    for exp in experiments:
        ...
        max_attempts = exp.get("max_attempts", 10)
        full_attempts = 0
        full_time_accum = 0.0
        best_full = None
        ...
        while not full_done and iter_count < max_attempts:
            ...
            # Primo embedding 1-to-1 (chain length = 1)
